...
def productdynamicsearchajax(request):
    ...
    try:
        ...
        product_name_searched = PProduct_Creation.objects.filter(Q(PProduct_SKU__icontains=product_name_typed) | Q(PProduct_color__contains=product_name_typed) | Q(Product__Product_Name__contains=product_name_typed)).values('Product__Product_Name','PProduct_SKU') #.annotate(total_qty= Sum('shades__godown_shades__quantity'))
        
        if product_name_searched:

            print(product_name_searched)
