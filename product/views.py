...
def definesubcategoryproduct(request, pk=None):
    ...
    formset = sub_category_and_bin_formset(queryset = bin_queryset, form_kwargs={'sub_cat_instance': instance}) # pass this(form_kwargs) to form and then to formset
...
def product2item(request,product_refrence_id):
    ...
    try:
        ...
        # allow clone ajax if there are no common or special items
                ...
                try:
                    with transaction.atomic():
                        ...
                        for form in formset:
                            ...
                            obj.quantity = quantity_to_add + approved_qty_differ  # instead of form_instance.approved_qty use difference qty 
...
def purchaseorderlabourworkinapprovecheckajax(request):
    ...
    if labour_workin_key:
        try:
            ...
            # Step 2: Group records by `unique_id`
            ...
            # Step 3: Convert grouped reports to a list of dictionaries
...
def finished_goods_godown_product_ref_wise_report(request, ref_no):
    ...
    if ref_no:
        ...
        #query for displaying pending_for_approval qty of a ref no 
...
def finished_goods_vendor_model_wise_report(request, ref_no, challan_no):
    ...
    if ref_no is not None and challan_no is not None:
        ...
        for x in SKU_List:
            ...
            sku = str(x.split('-')[0])  # Ensure the SKU is a string, if necessary
        ...
        for instance in labour_workin_instances:
            ...
            for x in SKU_List:
                ...
                sku = int(x.split('-')[0])  #Ensure the SKU is a int, if necessary
                
                if sku in sku_to_processed_pcs: #change from querylist to python list

                    str_sku =  str(sku)  # while creating a new dict save sku as STR but search sku_to_processed_pcs dict by int only 
                    
                    product_qty[str_sku] = sku_to_processed_pcs[sku] #product_qty[sku] current sku as key and sku_to_processed_pcs[sku] value of that sku from sku_to_processed_pcs
        ...
        #     skus = x.get('sku_qty')
        #     list_dict.append(skus)


        # SKU_List1 = []
        # for sku_instance in product_instance.productdetails.all().order_by('PProduct_SKU'):
        #     SKU_List1.append(str(sku_instance.PProduct_SKU))
        ...
        # final_dict = {}
        # for key, value in labour_workout_p_2_i.items():
        #     sum1 = result_dict.get(key)
        #     final_dict[key] = value - sum1
...
def raw_material_estimation_popup(request, pk=None):
    ...
    if pk:
        try:
            ...
            # request.POST for submitting post request form , initial for initializing form of initial data when creating,  instance of parent  for initializing saved data 
            ...
            return render(request, '404.html')  # or another appropriate error response
        
        except Exception as e:
            logger.exception("An unexpected error occurred while setting up the formsets.")
            return render(request, '500.html')  # or another appropriate error response
    ...
    if request.method == 'POST':
        ...
        if product_2_item_formset.is_valid() and material_product_estimation_items_formset.is_valid():
            try:
                ...
                return render(request, '500.html')  # or handle it according to your needs
        ...
        if estimation_master_instance:
            ...
            for key, value in final_total_list.items():
                ...
                try:
                    ...
                    difference_quantity_in_p_o_stage = purchase_order_for_raw_material.objects.filter(material_name=key).aggregate(total_po_qty=Sum('total_comsumption')) # example of aggregate {'total_po_qty': Decimal('5669.545')}
...
#__________________________Finished-Goods_start________________________________
            ...
            try:
                with transaction.atomic():
                    ...
                    for form in product_purchase_voucher_items_formset_instance:
                        ...
                        if form.instance.pk and not form.cleaned_data.get('DELETE'): # check if instance is on update mode
                            ...
                            old_quantity = form.initial.get('quantity_total')  # get the value before updating from this

                            # check if product_name is present in form.changed_data and form is form.has_changed()
                            if form.has_changed() and 'product_name' in form.changed_data: 

                                old_product_name = form.initial.get('product_name') # get the value before updating from this
                                # new_product_name = form.cleaned_data.get('product_name') # get the current value entered from this 
                                ...
                                if old_product_name and old_quantity:
                                    ...
                                    obj1.quantity = obj1.quantity - old_quantity # table qty - old qty
                                    obj1.save()
                                    ...
                                    obj2.quantity = obj1.quantity + product_purchase_voucher_items_form.quantity_total # table qty + new qty
                                    obj2.save()

                            else:  # if only qty is changed in form 

                                diff_qty_to_deduct = product_purchase_voucher_items_form.quantity_total - old_quantity # get the diff qty
                                ...
                                if created:
                                    ...
                                    obj.quantity = obj.quantity + diff_qty_to_deduct # table qty + diff qty 

                                obj.save()

                            product_purchase_voucher_items_form.save()

                        elif not form.instance.pk: # if form is on create 
                            if not form.cleaned_data.get('DELETE'):
                                ...
                                if created:
                                    ...
                                    obj.quantity = obj.quantity + product_purchase_voucher_items_form.quantity_total # table qty + new qty
...
def process_serial_no(request):
    if request.method == 'POST':
        ...
        if serial_no:  #Example logic
            ...
            # try:
            #     voucher_number = request.POST.get('instance_number_post')
            ...
            try:
                ...
                if response_data.get('response_code') == 200 and response_data.get('response_desc') == 'success':
                    ...
                    # itertools.chain.from_iterable is an efficient way to flatten lists of lists or iterables.
                    # It takes an iterable of iterables (e.g., list of QuerySets) and combines them into a single iterable.
                    # or flattened_list = [item for qs in querysets for item in qs]
...
#__________________________Finished-Goods_end________________________________


#___________________________warehouse_bin_start________________________________
...
def edit_rack_in_zone(request,rack_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            # return redirect('edit-rack-in-zone', rack_id=rack_id)
...
#__________________________warehouse_bin_end_____________________________________
...
def finished_goods_stock_all(request,pk=None):
    ...
    if pk:
        ...
        finished_godown_all = PProduct_Creation.objects.annotate(total_warehouse_stock = Sum( 
            'product_warehouse_quantity_through_table__quantity')).order_by('Product__Product_Name').select_related('PProduct_color') # all(). and .annotate on objects behaves in the same
...
def qc_approved_model_wise_report(request,ref_id):
    ...
    if ref_id:
        ...
        for record in l_w_in_instances:
            ...
            lwi_p_2_i = dict(record.l_w_in_products.all().values_list('product_sku','return_pcs')) # dict() to convert queryset to dict

            lwi_p_2_i_approved = dict(record.l_w_in_products.all().values_list('product_sku','approved_qty'))

            # values_list without dict() -> [<QuerySet [(40051011046, 500), (40051021046, 500)]>]
            # values_list with dict() -> [{40051011046: 500}]
            

            # with values without list()('product_sku','return_pcs') ->  [<QuerySet[{'product_sku': 40051041046, 'return_pcs': 100}]]
            # # with values with list() -> [{'product_sku': 40051041046, 'return_pcs': 100}]


            # with values_list('product_sku','return_pcs') - > qs of {40051041046: 100} key and value
            # with values() ('product_sku','return_pcs') ->  qs of {'product_sku' : 40051041046, 'return_pcs': 100}
...
def finished_goods_sorting_list(request):
    ...
    # Use itemgetter when working with dictionaries, tuples, or lists, and you want to retrieve a specific key or index. # ex - dicts
    # Use attrgetter to access attributes of objects (like class attributes or properties). ex - queryset attributes
