...
class raw_material_production_total(models.Model):
    ...
    #TODO
    # stock_in_cutting_stage  = models.DecimalField(max_digits=10, decimal_places=DECIMAL_PLACE_CONSTANT)
    # stock_in_labour_workout_stage  = models.DecimalField(max_digits=10, decimal_places=DECIMAL_PLACE_CONSTANT)
...
class finished_product_warehouse_bin(models.Model):
    ...
    def save(self, *args, **kwargs):
        # Only perform the check if this is an update (i.e., instance exists in the database)
        if self.pk:

            # Retrieve the current instance from the database
            current_instance = finished_product_warehouse_bin.objects.get(pk=self.pk)
            current_bin_size = current_instance.product_size_in_bin  # Original size
            
            # Count current number of child allocated products
            current_bin_count = self.finishedgoodsbinallocation_set.count()

            # If the new bin size is smaller than the current item count, raise an error
...
class finishedgoodsbinallocation(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        # Proceed with saving if the bin has space
        super().save(*args, ** kwargs)
