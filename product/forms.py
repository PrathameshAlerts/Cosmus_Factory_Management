...
class PProductCreateFormset(BaseInlineFormSet): # for BaseModelFormSet modelformset_factory and BaseInlineFormSet for inlineformset factory
...
class subcat_and_bin_form(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        self.sub_cat_instance = kwargs.pop('sub_cat_instance', None)  # Extract sub_cat_instance so that i can use it in FinishedProductWarehouseBinFormSet
        super().__init__(*args, **kwargs)



class FinishedProductWarehouseBinFormSet(BaseModelFormSet): # for BaseModelFormSet modelformset_factory and BaseInlineFormSet for inlineformset factory

    def __init__(self, *args, **kwargs):
        self.sub_cat_instance = kwargs.pop('form_kwargs', None)  # Extract instance from kwargs
        ...
        for form in self.forms:
            if form.instance:
                ...
                print(sub_cat_instance.sub_catergory_id)
                print(self.sub_cat_instance)

                

                if sub_cat_instance.sub_catergory_id == self.sub_cat_instance and self.sub_cat_instance is not None: # to check if instance belong to current parent and instance is not none 
                    form.fields['check_if_added'].initial = True  
                    form.fields['check_if_added_all'].initial = True   
                else:                                                  # to check if instance does not belong to current parent 
...
# check_if_added                                                check_if_added_all
# True   -> Checkbox tick same parent instance                   True   ->   Can Edit 
# False  -> Checkbox tick diff color another parent instance     False  ->   Cannot Edit
# False  -> not checked                                          True    ->   Can Edit 
